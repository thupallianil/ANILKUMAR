...
const Appliances = () => {
  ...
  const fetchProducts = async () => {
    setLoading(true);
    try {
      const params = { category: "Appliances" };
      if (subcategory) {
    ...
    <div>
      <div className="mb-6">
        <h2 className="text-3xl font-bold text-gray-800">
          Home & Appliances {subcategory && <span className="text-orange-600">› {subcategory}</span>}
        </h2>
        {subcategory && (
          <Link to="/products/Appliances" className="text-orange-500 hover:underline text-sm mt-1 inline-block">
            ← View All Appliances
          </Link>
        )}
      </div>
      ...
      {loading ? (
        ...
        <div className="text-center py-12">
          ...
          <Link to="/products/Appliances" className="text-orange-600 hover:underline mt-2 inline-block">
          ...
          {products.map((product) => (
            ...
            >
              ...
              <Link to={`/product/${product.id}`}>
                <h3 className="text-lg font-semibold text-center hover:text-orange-600 transition-colors">{product.name}</h3>
              </Link>
