...
const Beauty = () => {
  ...
  const fetchBeauty = async () => {
    setLoading(true);
    const res = await getProducts({ category: 'beauty' });
    if (res.success) {
      const productList = res.data.results || res.data || [];
      setProducts(productList);
    } else {
      console.error("Failed to fetch beauty products:", res.error);
      setProducts([]);
    }
    setLoading(false);
  };
  ...
  return (
    <div>
      <Header />
      <div className="p-6">
        <h2 className="text-2xl font-bold mb-4">Beauty & Personal Care</h2>
        {loading ? (
